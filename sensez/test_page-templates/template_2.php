...
<div class="step-main-wrapper">
	<div class="test-progress">
		...
		<?php if ($field = get_previous_post(true, '', 'gender') ?: get_post(324)): ?>
		<a href="<?php the_permalink($field->ID) ?>" class="btn-previous btn-previous-active">
...
<div class="page-content">
										...
										<span>
											<figure>
												<div class="flame-lightblue">
													<lottie-player class="hover" src="<?= get_stylesheet_directory_uri() ?>/lottie/hoverlightblue.json" style="width: 100%; height: auto"></lottie-player>
													<lottie-player class="click" src="<?= get_stylesheet_directory_uri() ?>/lottie/clicklightblue.json" style="width: 100%; height: auto"></lottie-player>
												</div>
												<div class="flame-blue">
													<lottie-player class="hover" src="<?= get_stylesheet_directory_uri() ?>/lottie/hoverblue.json" style="width: 100%; height: auto"></lottie-player>
													<lottie-player class="click" src="<?= get_stylesheet_directory_uri() ?>/lottie/clickpurple1.json" style="width: 100%; height: auto"></lottie-player>
												</div>
	...
	<?php
	$featured_posts = get_field('questions');
	if($featured_posts): ?>
				...
				<div class="test-scroller">
					<ul>
						...
						<?php foreach($featured_posts as $index => $post): 
							...
							setup_postdata($post); ?>
							<li>
								<input type="hidden" post_id="<?= $post->ID ?>" readonly />
								<div class="image">
									<?php the_post_thumbnail('full') ?>
								</div>
								<h3><?php the_title() ?></h3>
